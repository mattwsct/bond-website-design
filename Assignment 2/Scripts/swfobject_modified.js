...
const swfobject = (function () {
  ...
  const regObjArr = [];
  let timer = null;
  let storedAltContent = null;
  let storedAltContentId = null;
  ...
  function showExpressInstall(regObj) {
    ...
    const obj = getElementById(regObj.id);
    if (obj) {
      if (regObj.altContentId) {
        const ac = getElementById(regObj.altContentId);
        if (ac) {
          storedAltContent = ac;
  ...
  function setVisibility(id, isVisible) {
    const v = isVisible ? 'inherit' : 'hidden';
    if (isDomLoaded) {
  ...
  return {
    /* Public API
		...
		*/
    registerObject(objectIdStr, swfVersionStr, xiSwfUrlStr) {
      if (!ua.w3cdom || !objectIdStr) {
        return;
      }
      const obj = document.getElementById(objectIdStr);
      const xi = getExpressInstall(obj);
      const regObj = {};
    ...
    getObjectById(objectIdStr) {
      ...
      if (ua.w3cdom && isDomLoaded) {
        ...
        if (o) {
          const n = o.getElementsByTagName(OBJECT)[0];
    ...
    embedSWF(
      ...
      if (hasPlayerVersion(swfVersionStr)) {
        ...
        const par = typeof parObj === OBJECT ? parObj : {};
        if (typeof flashvarsObj === OBJECT) {
          for (const i in flashvarsObj) {
    ...
    getQueryParamValue(param) {
      const q = doc.location.search || doc.location.hash;
      if (param == null) {
        return q;
      }
      if (q) {
